...
# accelerometers from the Samsung Galaxy S smartphone. A full description is available at the site where
...
# Reading 6 Activity Label Names
activity_labels <- read.table("./UCI HAR Dataset/activity_labels.txt", header = FALSE)


# Reading 561 Features Name
...
# Renaming columns in activity_dat & activity_labels dataframes
...
# Renaming subject_dat columns
names(subject_dat) <- "Subject"

# Renaming feature_dat columns using columns from features_name
names(feature_dat) <- features_name$V2


# Create one large Dataset with only these variables: subject_dat,  Activity,  feature_dat
Large_dataset <- cbind(subject_dat, Activity,feature_dat)
View(Large_dataset)
# Creating New datasets by extracting only the measurements on the mean and
# standard deviation for each measurement.
features_name_mean_std <- features_name$V2[grep("\\bmean()\\b|\\bstd()\\b", features_name$V2)]
View(features_name_mean_std)
...
# Rename the columns of the large_dataset using more descriptive activity names
